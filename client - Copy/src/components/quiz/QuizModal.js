...
const QuizModal = ({ quiz, onClose }) => {
  ...
  const handleAnswerChange = (qIdx, answerIdx) => {
    ...
    setAnswers((prev) => 
      prev.map((ans, i) => i === qIdx ? { selectedAnswerIndex: answerIdx } : ans)
    );
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const message = err.response?.data?.data?.message || "Տեղի է ունեցել սխալ:";
      const isAlreadyTaken = err.response?.status === 400 || message.includes("already");
      ...
      <div className="modal-card">
        ...
        <header className="modal-header">
          ...
          <div className="progress-pill">
            {answers.filter(a => a.selectedAnswerIndex !== null).length} / {quiz.questions.length} Լրացված
          </div>
        </header>
          ...
          {score === null ? (
            quiz.questions.map((q, qIdx) => (
              <div key={qIdx} className="question-section">
                <p className="question-text">{qIdx + 1}. {q.text}</p>
                <div className="options-list">
                  {q.options.map((opt, oIdx) => (
                    ...
                    >
                      ...
                      />
                      <span className="option-bullet">{String.fromCharCode(65 + oIdx)}</span>
                      {opt}
                    </label>
                  ))}
                </div>
        ...
        <footer className="modal-footer">
          ...
          {score === null && (
            ...
            >
              {loading ? <><FaSpinner className="spin" /> Ուղարկվում է...</> : "Հաստատել արդյունքը"}
            </button>
          )}
        </footer>
      </div>
