...
import { FaSearch, FaCloudUploadAlt, FaFileExcel, FaCheckCircle, FaExclamationCircle, FaSpinner } from "react-icons/fa";
...
const UserLabsList = () => {
  ...
  const handleSubmit = async (e, labId) => {
    ...
    if (!file) {
      setUploadStates(prev => ({ ...prev, [labId]: { status: 'error', message: 'Ընտրեք ֆայլը' } }));
      return;
    }
    ...
    try {
      ...
      setUploadStates(prev => ({ 
        ...
        [labId]: { 
          status: 'error', 
          message: isAlreadySubmitted ? "Արդեն տեղադրված է" : "Սխալ վերբեռնման ժամանակ" 
        } 
      }));
    }
  };
    ...
    <div className="labs-page-wrapper">
      <div className="labs-container">
        ...
        {loading ? (
          ...
          <div className="error-card"><FaExclamationCircle /> {error}</div>
        ) : (
          <div className="labs-grid">
            {filteredLabs.length === 0 ? (
              ...
              filteredLabs.map((lab) => (
                <div key={lab._id} className="lab-card" ref={(el) => (labRefs.current[lab._id] = el)}>
                  <div className="lab-info">
                    <h3 className="lab-title">{lab.title}</h3>
                    <p className="lab-content">{lab.content}</p>
                  </div>

                  <form className="lab-upload-form" onSubmit={(e) => handleSubmit(e, lab._id)}>
                    <label className={`file-drop-zone ${fileInputs[lab._id] ? 'file-selected' : ''}`}>
                      <input
                        type="file"
                        accept=".xlsx,.xls"
                      ...
                      <span>
                        {fileInputs[lab._id] ? fileInputs[lab._id].name : "Ընտրեք  ֆայլը"}
                      </span>
                    </label>
                    ...
                    <button 
                      ...
                      disabled={uploadStates[lab._id]?.status === 'uploading' || uploadStates[lab._id]?.status === 'success'}
                    >
                      {uploadStates[lab._id]?.status === 'uploading' ? 'Տեղադրվել է' : 'Տեղադրել պատասխանը'}
                    </button>
                  </form>

                  {uploadStates[lab._id]?.status === 'success' && (
                    <div className="status-msg success"><FaCheckCircle /> Հաջողությամբ ուղարկված է</div>
                  )}
                  {uploadStates[lab._id]?.status === 'error' && (
                    <div className="status-msg error"><FaExclamationCircle /> {uploadStates[lab._id].message}</div>
                  )}
                </div>
              ))
            )}
