...
const AdminLessons = () => {
  ...
  const handleUpdate = async (id) => {
    ...
    try {
      ...
      setResources((prev) => prev.map((item) => (item._id === id ? data : item)));
      setEditingId(null);
      setNewTitle("");
    } catch (err) {
                  ...
                  {editingId === item._id ? (
                    <div className="edit-form">
                      ...
                      <div className="edit-btns">
                        <button className="btn-save" onClick={() => handleUpdate(item._id)}>Պահպանել</button>
                        <button className="btn-cancel" onClick={() => setEditingId(null)}>Չեղարկել</button>
                      </div>
                    </div>
                  ) : (
                    <div className="view-row">
                      ...
                      <div className="actions">
                        <button className="btn-edit" onClick={() => { setEditingId(item._id); setNewTitle(item.title); }}>Խմբագրել</button>
                        <button className="btn-del" onClick={() => handleDelete(item._id)}>Ջնջել</button>
                      </div>
                    </div>
                  )}
                </div>
